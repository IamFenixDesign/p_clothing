...
local function applyLastEquipped()
...
local function clearLastEquipped()
    lastEquipped = {}
    syncNuiToggles()
end
...
local function toggleOpen()
    if not openState and IsNuiFocused() then return end
...
--[[
  Appearance-script compatibility (illenium-appearance, ox_inventory, dpclothing API)

  dpc:EquipLast     — put toggled clothes back on and clear the cache
                      (call before clothing stores / appearance menus)
  dpc:ResetClothing — clear the cache only (call after a new outfit is applied)
  dpc:Menu          — open/close the radial menu
  dpc:ToggleMenu    — toggle the radial menu
]]

local function equipLast()
    closeMenu()
    applyLastEquipped()
    lastEquipped = {}
    syncNuiToggles()
...
local function setMenu(status)
    if status then
        if not openState then toggleOpen() end
    else
        closeMenu()
    end
end

RegisterNetEvent("dpc:EquipLast", equipLast)
RegisterNetEvent("p-clothing:client:equipLast", equipLast)

RegisterNetEvent("dpc:ResetClothing", clearLastEquipped)
RegisterNetEvent("p-clothing:client:resetClothing", clearLastEquipped)

RegisterNetEvent("dpc:Menu", setMenu)
RegisterNetEvent("p-clothing:client:setMenu", setMenu)

RegisterNetEvent("dpc:ToggleMenu", toggleOpen)

exports("EquipLast", equipLast)
exports("ResetClothing", clearLastEquipped)
exports("SetMenu", setMenu)
exports("ToggleMenu", toggleOpen)
